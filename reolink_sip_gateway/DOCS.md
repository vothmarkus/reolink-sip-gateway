# Reolink SIP Gateway 1.5.0 – Dokumentation

## Zweck

**1.5.0** aktualisiert den Reolink-NVR-Katalog beim Start und alle fünf Minuten, behält den letzten erfolgreichen Stand bei Ausfällen und stellt für Kameras mit UID einen kanalunabhängigen, FRITZ!Fon-kompatiblen `.jpg`-Link bereit. Ein akzeptiertes Klingelereignis lädt das Türbild parallel zum Rufaufbau vor. Die Ingress-Seite zeigt Nummer, Kameraname, Onlinezustand, Bildquelle, Zeitpunkte, Kopierknöpfe und Browsertests. Der bestehende Tür-Livebild-Link und die nummerierten Links aus 1.4 bleiben unverändert; Klingeltaster-Routen, beide SIP-Konten und der einzige Reolink-Audioweg werden nicht verändert.
...
Die sichtbare Reihenfolge bleibt **Reolink**, **SIP-Telefonie**, **Audio**, **Anruf**, **Anrufrouten / Klingeltaster**, **FRITZ!Fon-Livebilder** und **Betrieb & Diagnose**. 1.5.0 ändert weder Schlüssel, gespeicherte Optionswerte, Defaults noch Runtimeübersetzung.
...
2. Persistente API-Instanz-ID, API-Token und unabhängiges Livebild-Pfadtoken laden beziehungsweise beim ersten Start sicher erzeugen; im NVR-/Auto-Modus den letzten Livebild-Kamerakatalog laden.
...
4. Bei NVR-/Auto-Modus und aktivierten Livebildern im Hintergrund die Kanäle samt Namen, Onlinezustand und UID erkennen; die Prüfung alle fünf Minuten wiederholen und bei einem Fehler den letzten Katalog behalten.
...
Ist `live_image.fritzfon_live_image_enabled` aktiv, registriert der bestehende Statusserver auf Port `18099` weiterhin den geheimen Türpfad `/fritzfon/<token>.jpg`. In NVR- und Auto-Modus fragt das Gateway beim Start und danach alle fünf Minuten die offizielle Reolink-Funktion `GetChannelstatus` ab. Jeder erkannte Kanal bleibt unter dem kompatiblen Pfad `/fritzfon/<token>/channel-<Nummer>.jpg` erreichbar. Die Nummer ist wie in der App 1-basiert; ein interner Reolink-Kanal `0` erscheint somit als `channel-1.jpg`. Der konfigurierte Hauptkanal wird immer angeboten. Im Standalone-Modus bleibt es beim Hauptkanal.

Enthält der Kanalstatus eine Reolink-UID, normalisiert das Gateway sie intern und bildet mit SHA-256 daraus eine 128-Bit-Kamera-ID. Der zusätzliche empfohlene Pfad `/fritzfon/<token>/camera-<id>.jpg` folgt dieser Kamera bei einem späteren Kanalwechsel. Weder Roh-UID noch Reolink-Zugangsdaten erscheinen in der URL oder Oberfläche. Doppelte oder syntaktisch ungültige IDs werden nicht veröffentlicht.

Die Ingress-Seite zeigt die vollständige lokale Adresse ohne Protokollpräfix sowie Browsertests für Tür-, Kamera- und nummerierte Kanalpfade. Hinzufügen, Entfernen, Umbenennen und Verschieben eines NVR-Kanals wird spätestens nach fünf Minuten sichtbar; ein Neustart ist nicht nötig. Ein fehlgeschlagener Abruf ersetzt den letzten erfolgreichen Katalog nicht. Die Oberfläche zeigt letzten Erkennungsversuch/-erfolg und je Kanal online/offline sowie letzten Bildabruf, HTTPS-/HTTP-/RTSP-Quelle und Dauer. Das Gateway ermittelt die lokale Home-Assistant-IPv4-Adresse anhand der Route zum SIP-Registrar, ohne ein Paket zu senden. Schlägt die Ermittlung fehl, erscheint der eindeutige Platzhalter `HOME-ASSISTANT-IP`, der durch die tatsächliche lokale Adresse des Home-Assistant-Hosts ersetzt werden muss.
...
1.5.0 ergänzt keine Option und benötigt keine Datenmigration. Der letzte Kamerakatalog wird nach einer erfolgreichen Erkennung automatisch neu angelegt; Löschen dieser Datei erzwingt gefahrlos eine frische Erkennung. Der Türpfad, alle nummerierten 1.4-Pfade, gespeicherte Optionswerte und die Gruppenreihenfolge bleiben unverändert.
