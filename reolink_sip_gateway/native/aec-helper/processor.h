...
// Shared by the Linux pipe helper and Android JNI. Identical WebRTC APM
// settings, 8 kHz / 10 ms PCM frames, and AEC1/AER1 diagnostics on both hosts.
namespace reolink_aec {
...
inline webrtc::AudioProcessing::Config::NoiseSuppression::Level NoiseLevel(
...
inline WireStats MakeWireStats(const webrtc::AudioProcessingStats& stats) {
  WireStats out;
...
inline void EncodeReply(int32_t status, const WireStats& stats,
...
class Processor {
 public:
  Processor(bool high_pass, bool noise_suppression, const std::string& noise_level) {
    apm.reset(webrtc::AudioProcessingBuilder().Create());
    if (!apm) {
      throw std::runtime_error("AudioProcessingBuilder::Create failed");
    }
    ...
    if (const int init = apm->Initialize(); init != webrtc::AudioProcessing::kNoError) {
      throw std::runtime_error("AudioProcessing::Initialize failed status=" + std::to_string(init));
    }

  }
  void Process(const std::array<uint8_t, kRequestBytes>& request,
               std::array<uint8_t, kReplyBytes>& reply) {
    if (std::memcmp(request.data(), "AEC1", 4) != 0)
      throw std::invalid_argument("invalid AEC request magic");
    const webrtc::StreamConfig stream(kSampleRate, 1);
    std::array<int16_t, kFrameSamples> render{}, render_out{}, capture{}, processed{};
    size_t off = 4;
