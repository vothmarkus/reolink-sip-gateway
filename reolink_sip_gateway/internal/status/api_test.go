...
func TestAPIV1RequiresBearerToken(t *testing.T) {
	...
	if info.APIVersion != APIVersion || info.GatewayVersion != "1.2.2" || info.InstanceID != testInstanceID {
...
func TestAPIV1RouteAvailabilityUsesEachRoutesAccounts(t *testing.T) {
	snapshot := Snapshot{
		Version: "1.2.2", State: "idle", DoorCallEnabled: true, SIPRegistered: true,
...
func TestAPIV1MobileOnlyRegistrationIsAvailable(t *testing.T) {
	status := newAPIStatus(Snapshot{
		Version: "1.2.2", State: "idle", ParallelCallEnabled: true, ParallelSIPRegistered: true,
	}, nil)
