...
func buildLoginXML(userHash, passwordHash string) ([]byte, error) {
	return marshalXMLDocument(xmlLoginBody{
		LoginUser: xmlLoginUser{Version: "1.1", UserName: userHash, Password: passwordHash, UserVer: 1},
		LoginNet:  xmlLoginNet{Version: "1.1", Type: "LAN", UDPPort: 0},
	})
}

func buildPreviewXML(channel uint8, handle uint32, stream Stream) ([]byte, error) {
	return marshalXMLDocument(xmlPreviewBody{Preview: xmlPreview{
		Version: "1.1", ChannelID: channel, Handle: handle, StreamType: stream,
	}})
}

func buildStopPreviewXML(channel uint8, handle uint32) ([]byte, error) {
	return marshalXMLDocument(xmlPreviewStopBody{Preview: xmlPreviewStop{
		Version: "1.1", ChannelID: channel, Handle: handle,
	}})
}
...
func trimXML(buf []byte) string { return strings.TrimSpace(string(buf)) }
func parseEncryptLen(xmlText string) (int, bool) {
	if xmlText == "" {
		return 0, false
	}
	var body xmlEncryptLenBody
	if err := xml.Unmarshal([]byte(xmlText), &body); err != nil || body.EncryptLen <= 0 {
		return 0, false
	}
